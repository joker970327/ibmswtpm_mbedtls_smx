		     ...
		     )
{
    ...
    switch(scheme)
	{
	  ...
	  case TPM_ALG_SM2:
	    ...
	    // return TPM_RC_SUCCESS;
		LoadEccParameter(&testSig.signature.sm2.signatureR, &c_TestSm2_r);
	    LoadEccParameter(&testSig.signature.sm2.signatureS, &c_TestSm2_s);
	    break;
	  default:
	    SELF_TEST_FAILURE;
	    break;
	...
	if(scheme == TPM_ALG_SM2){ 
	    testObject.publicArea.parameters.eccDetail.curveID = TPM_ECC_SM2_P256;
		MemoryCopy2B(&testObject.sensitive.sensitive.ecc.b, &c_sm2TestKey_ds.b,
			sizeof(testObject.sensitive.sensitive.ecc.t.buffer));
		LoadEccPoint(&testObject.publicArea.unique.ecc, &c_sm2TestKey_QsX,
			&c_sm2TestKey_QsY);

		if(TPM_RC_SUCCESS != CryptEccValidateSignature(&testSig, &testObject,
							(TPM2B_DIGEST *)&c_sm2TestValue.b))
		{
			SELF_TEST_FAILURE;
		}
		CHECK_CANCELED;
