...
#if defined(__ALTERA_ONCHIP_FLASH)
#include <altera_common.h>
#include <sys/alt_flash.h>
static alt_flash_fd*       s_NvFile = NULL;
static int                 s_NeedsManufacture = FALSE;
#endif
...
#   endif
	...
	// s_NvFile = alt_flash_open_dev("/dev/onchip_flash_data");
	return (s_NvFile == NULL) ? -1 : 0;
}
...
#elif defined(__ALTERA_ONCHIP_FLASH)
/* This function opens the file used to hold the NV image.
   Return Type: int */
/* Return Value	Meaning */
/* >=0	success */
...
{
	s_NvFile = alt_flash_open_dev(ONCHIP_FLASH_DATA_NAME);
	return (s_NvFile == NULL) ? -1 : 0;
}
...
{
	int OK;
	OK = (0 == alt_write_flash(s_NvFile, 0, s_NV, NV_MEMORY_SIZE));
    assert(OK);
    return OK;
}
		...
		)
{
    ...
    if(NvFileOpen("r+b") >= 0)
	{
	    ...
	    if(NV_MEMORY_SIZE == fileSize)
		{
		    s_NeedsManufacture = alt_read_flash(s_NvFile, 0,s_NV, NV_MEMORY_SIZE);
		}
	    else
		{
		    NvFileCommit();     // for any other size, initialize it
		 ...
		 )
{
    ...
    if(NULL != s_NvFile)
	{
	    alt_flash_close_dev(s_NvFile);    // Close NV file
	    // Alternative to deleting the file is to set its size to 0. This will not
	    // match the NV size so the TPM will need to be remanufactured.
	    if(delete)
		{
